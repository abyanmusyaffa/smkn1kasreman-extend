<a href="/jobfair/{{ $slug }}" wire:navigate class="w-full group">
    <article class="flex w-full lg:flex-col border-2 lg:border-4 border-blue-100 rounded-2xl gap-2 bg-white lg:group-hover:scale-105 lg:group-hover:shadow-xl duration-500 transition-all">
        ...
        <figcaption class="w-2/3 lg:w-full flex flex-col gap-2 py-2 pr-2 lg:px-2 lg:pb-2 lg:items-center justify-between">
            ...
            <div class="flex w-fit px-1 lg:px-2 py-0.5 lg:py-1 rounded items-center gap-1 {{ \Carbon\Carbon::now()->greaterThan(\Carbon\Carbon::parse($deadline)) ? 'bg-red-100' : 'bg-blue-100' }}">
